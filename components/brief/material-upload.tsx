"use client";
import { type ClipboardEventHandler, useRef, useState } from "react";
...
export function MaterialUploadField({ value, onChange, urlInput, setUrlInput, projectId = "temp-project" }: { value: UploadItem[]; onChange: (v: UploadItem[]) => void; urlInput: string; setUrlInput: (v: string) => void; projectId?: string }) {
  ...
  const uploadOne = async (file: File) => {
    ...
    onChange([...value, { id: localId, file_name: file.name, file_size: file.size, file_type: file.type, kind: isImage ? "image" : "file", preview: isImage ? URL.createObjectURL(file) : undefined, status: "uploading", progress: 30 }]);
    ...
    try {
      ...
      onChange((prev => prev.map((v:any) => v.id === localId ? { ...v, ...data.file, status: "done", progress: 100 } : v)) as any);
    } catch { onChange((prev => prev.map((v:any) => v.id === localId ? { ...v, status: "error" } : v)) as any); }
  };
  ...
  const remove = async (item: UploadItem) => {
    if (item.kind === "url") return onChange(value.filter((v) => v.id !== item.id));
